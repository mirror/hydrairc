   ...
   LRESULT OnParentCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
   {
      ...
      else if( pnmh->hwndFrom == GetParent() ) 
      {
         ...
         else if( lpCustomDraw->dwDrawStage == CDDS_ITEMPREPAINT ) 
         {
            ...
            // Hydra - begin: get the Name of the taskbar and display it if it has one
            int nCurrentToolbar = ::SendMessage(pnmh->hwndFrom, RB_IDTOINDEX, lpCustomDraw->dwItemSpec, NULL);
            if (nCurrentToolbar != -1)
            {
              TCHAR szText[_nMaxMenuItemTextLength] = { 0 };
              REBARBANDINFO rbbi = { sizeof(REBARBANDINFO), RBBIM_TEXT | RBBIM_CHILD | RBBIM_STYLE | RBBIM_COLORS };
              rbbi.lpText = szText;
              rbbi.cch = sizeof(szText)/sizeof(TCHAR);

              BOOL bRet = (BOOL)::SendMessage(pnmh->hwndFrom, RB_GETBANDINFO, nCurrentToolbar, (LPARAM)&rbbi);

              dc.FillSolidRect(&rc, rbbi.clrBack);
              ::InflateRect(&rc, -2, -4);
              ...
              if( ::lstrlen(szText)>0 ) {
                ...
                UINT uTextFlags = DT_RIGHT;
                COLORREF clrText;
                clrText = m_xpstyle.clrButtonText;

                uTextFlags |= DT_SINGLELINE | DT_VCENTER;
                RECT rcText = rc;

                HFONT oldfont = dc.GetCurrentFont();
                HFONT newfont = (HFONT)::SendMessage(pnmh->hwndFrom,WM_GETFONT,0,0);
                dc.SetBkMode(TRANSPARENT);
            ...
            rc.left += 2;
            if( rc.left < rc.right ) {
              // Hydra - don't draw huuuuge grippers!
              if (abs(rc.right) - abs(rc.left) > 3)
                rc.right = rc.left + 3;
